...
export type HousekeepingTaskData = {
  id: string
  propertyId: string
  roomUnitId: string
...
export function HousekeepingTaskDialog({ open, onOpenChange, propertyId, task, onSuccess }: Props) {
  ...
  const roomUnitsQuery = useQuery({
    queryKey: ["hospitality", "hk-task", "ru", propertyId],
    queryFn: () =>
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/hospitality/housekeeping-tasks/${task.id}`, payload)
    } else {
      await api.post("/v1/hospitality/housekeeping-tasks", payload)
    }
    onSuccess()
  }
