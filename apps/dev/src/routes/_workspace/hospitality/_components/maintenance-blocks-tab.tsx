...
import { type MaintenanceBlockData, MaintenanceBlockDialog } from "./maintenance-block-dialog"
...
export function MaintenanceBlocksTab({ propertyId }: Props) {
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editing, setEditing] = useState<MaintenanceBlockData | undefined>()

  const { data, isPending, refetch } = useQuery({
    queryKey: ["hospitality", "maintenance-blocks", propertyId],
    queryFn: () =>
      api.get<ListResponse<MaintenanceBlockData>>(
        `/v1/hospitality/maintenance-blocks?propertyId=${propertyId}&limit=200`,
      ),
    enabled: !!propertyId,
  })

  const roomTypesQuery = useQuery({
    queryKey: ["hospitality", "maintenance-blocks", "rt", propertyId],
    queryFn: () =>
  ...
  const roomUnitsQuery = useQuery({
    queryKey: ["hospitality", "maintenance-blocks", "ru", propertyId],
    queryFn: () =>
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/v1/hospitality/maintenance-blocks/${id}`),
    onSuccess: () => {
      void refetch()
    },
  })
