...
const INVENTORY_MODES = ["pooled", "dedicated"] as const

type InventoryMode = (typeof INVENTORY_MODES)[number]
...
export function RoomTypeDialog({ open, onOpenChange, propertyId, roomType, onSuccess }: Props) {
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/hospitality/room-types/${roomType.id}`, payload)
    } else {
      await api.post("/v1/hospitality/room-types", payload)
    }
    onSuccess()
  }
