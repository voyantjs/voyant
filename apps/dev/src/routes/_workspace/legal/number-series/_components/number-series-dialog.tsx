...
export function NumberSeriesDialog({
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/admin/legal/contracts/number-series/${series.id}`, payload)
    } else {
      await api.post("/v1/admin/legal/contracts/number-series", payload)
    }
    onSuccess()
  }
