...
export type DropoffPriceRuleData = {
  id: string
...
export function DropoffPriceRuleDialog({ open, onOpenChange, rule, onSuccess }: Props) {
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/pricing/dropoff-price-rules/${rule.id}`, payload)
    } else {
      await api.post("/v1/pricing/dropoff-price-rules", payload)
    }
    onSuccess()
  }
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                {form.formState.errors.dropoffName && (
                  <p className="text-xs text-destructive">
                    {form.formState.errors.dropoffName.message}
                  </p>
                )}
              </div>
              <div className="flex flex-col gap-2">
