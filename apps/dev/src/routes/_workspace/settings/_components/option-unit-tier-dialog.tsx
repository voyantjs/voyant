...
export type OptionUnitTierData = {
  id: string
...
export function OptionUnitTierDialog({ open, onOpenChange, tier, onSuccess }: Props) {
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/pricing/option-unit-tiers/${tier.id}`, payload)
    } else {
      await api.post("/v1/pricing/option-unit-tiers", payload)
    }
    onSuccess()
  }
