...
export function PriceScheduleDialog({
  ...
  const [catalogs, setCatalogs] = useState<PriceCatalogOption[]>([])
  ...
  useEffect(() => {
    if (!open) return
    void api
      .get<PriceCatalogListResponse>("/v1/pricing/price-catalogs?limit=200")
  ...
  const onSubmit = async (values: ScheduleFormOutput) => {
    ...
    if (isEditing) {
      await api.patch(`/v1/pricing/price-schedules/${schedule.id}`, payload)
    } else {
      await api.post("/v1/pricing/price-schedules", payload)
    }
    onSuccess()
  }
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <DatePicker
                  ...
                  onChange={(v) => form.setValue("validFrom", v ?? "", { shouldDirty: true })}
              ...
              <div className="flex flex-col gap-2">
                ...
                <DatePicker
                  ...
                  onChange={(v) => form.setValue("validTo", v ?? "", { shouldDirty: true })}
                  placeholder="Optional"
                />
              </div>
            </div>
