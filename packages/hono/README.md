...
`@voyantjs/hono` itself is the default transport adapter, not an optional
integration plugin. Prefer the `HonoBundle` terminology for the
transport-specific bundle contribution format used by reusable packages that
need to add Hono modules, extensions, links, or subscribers. The older
`HonoPlugin` name remains as a compatibility alias.
...
| `./plugin` | Hono-specific bundle contribution surface: `HonoBundle`, `defineHonoBundle`, `expandHonoBundles` plus legacy `HonoPlugin` aliases |
