...
export function createContractsAdminRoutes(options: ContractsRouteOptions = {}) {
  return new Hono<Env>()
    .get("/templates", async (c) => {
      const query = await parseQuery(c, contractTemplateListQuerySchema)
      return c.json(await contractsService.listTemplates(c.get("db"), query))
    })
    .get("/templates/default", async (c) => {
      const query = await parseQuery(c, contractTemplateDefaultQuerySchema)
    ...
    .get("/", async (c) => {
      const query = await parseQuery(c, contractListQuerySchema)
