...
function initialsFrom(firstName: string | null, lastName: string | null): string {
  const f = firstName?.[0] ?? ""
  const l = lastName?.[0] ?? ""
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="grid flex-1 grid-cols-12 gap-4 p-4 lg:p-6">
        ...
        <main className="col-span-12 flex flex-col gap-4 lg:col-span-6">
          <Tabs
            value={activeTab}
            onValueChange={(v) => setActiveTab(v as "overview" | "notes" | "activities")}
          >
              ...
              <div className="grid grid-cols-3 gap-3">
                <Card size="sm">
                  <CardContent className="pt-4">
                    ...
                    <p className="text-2xl font-semibold">{openOpportunities.length}</p>
                  </CardContent>
                </Card>
                <Card size="sm">
                  <CardContent className="pt-4">
                ...
                <Card size="sm">
                  <CardContent className="pt-4">
                    ...
                    <p className="text-2xl font-semibold">{wonOpportunities.length}</p>
                  </CardContent>
                </Card>
              </div>
                      ...
                      {notes.slice(0, 3).map((note) => (
                        <NoteCard
                          ...
                          savingEdit={
                            ...
                            updateNoteMutation.variables?.noteId === note.id
                          }
                          deleting={
                          ...
                          onSave={(content) =>
                            updateNoteMutation
                              .mutateAsync({ noteId: note.id, content })
                              .then(() => undefined)
                          }
                          onDelete={() =>
                            deleteNoteMutation.mutateAsync(note.id).then(() => undefined)
                          }
                        />
                      ))}
                    </div>
                      ...
                      {activities.map((activity) => (
                        <ActivityTimelineItem
                          ...
                          savingEdit={
                            ...
                            updateActivityMutation.variables?.activityId === activity.id
                          }
                          deleting={
                            deleteActivityMutation.isPending &&
                            deleteActivityMutation.variables === activity.id
                          }
                          togglingStatus={
                            updateActivityMutation.isPending &&
                            updateActivityMutation.variables?.activityId === activity.id
                          ...
                          onDelete={() =>
                            deleteActivityMutation.mutateAsync(activity.id).then(() => undefined)
                          }
                          onToggleStatus={() =>
                            updateActivityMutation
                              .mutateAsync({
